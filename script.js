...
const boutonInputHeader = document.querySelector(".bouton-input-header"); // Récupération du bouton de soumission des filtres
const inputHeader = document.querySelector(".input-header"); // Récupération de l'input pour les filtres
const containerFilteredtTag = document.querySelector(".container-filtered-tag"); // Récupération du container pour les tags filtrés
...
function init() {
  ...
  // Affichage et activation des filtres pour les tags
  ...
  // Ajout des écouteurs d'événements pour les inputs des tags avant filtrage principal
...
function handleMainInputFilter() {
  ...
  } else if (filteredDataMainInput.length === 0) {
    resetDisplay();
  } else {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    inputSubit.forEach((input, index) => {
      input.addEventListener("click", (event) => {
        ...
        handleTagInputClick(event, index);
      });
    });
  });
}
...
// Fonction pour gérer le clic sur les inputs des tags
function handleTagInputClick(event, index) {
